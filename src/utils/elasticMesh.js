...
void main() {
  ...
  gl_FragColor = vec4(lit, alpha * uOpacity);
}
`;
...
export function initElasticMesh(container, options = {}) {
  const {
    image = '',
    color1 = '#5227FF',
    color2 = '#B19EEF',
  ...
  const targetEl = eventTarget || container;
  ...
  const program = new Program(gl, {
    ...
    uniforms: {
      ...
      uHighlight: { value: hexToRgb(props.highlight) },
      uGrid: { value: props.showGrid ? 1 : 0 },
      uGridDensity: { value: props.gridDensity },
      uGridOpacity: { value: props.gridOpacity },
      uGridColor: { value: hexToRgb(props.gridColor) },
      uShading: { value: props.shading },
      uRes: { value: [1, 1] },
      uRadius: { value: props.borderRadius },
      uAspect: { value: 1 },
      uTilt: { value: (props.tilt * Math.PI) / 180 },
      uDist: { value: DIST },
      uFit: { value: FIT },
  ...
  function toPlane(clientX, clientY) {
    ...
    const clipX = mx * 2 - 1;
    const clipY = 1 - my * 2;
    const t = ((props.tilt || 0) * Math.PI) / 180;
    const ct = Math.cos(t);
    const st = Math.sin(t);
  ...
  function onMove(e) {
    ...
    if (props.interaction === 'hover') pointer.targetActive = true;
  }
  function onEnter() {
    if (props.interaction === 'hover') pointer.targetActive = true;
  }
  ...
  function onUp() {
    if (props.interaction === 'drag') pointer.targetActive = false;
  }
  function onTouch(e) {
    if (e.touches.length) {
  ...
  targetEl.addEventListener('touchstart', onTouch, { passive: true });
  targetEl.addEventListener('touchmove', onTouch, { passive: true });
  targetEl.addEventListener('touchend', onLeave);
  ...
  return () => {
    ...
    targetEl.removeEventListener('touchstart', onTouch);
    targetEl.removeEventListener('touchmove', onTouch);
    targetEl.removeEventListener('touchend', onLeave);
